    ...
    initRoutes() {
        ...
        app.get('/sftp', function (req, res) {
            sftp.connect({
                ...
                port: '22',
                username: 'imrul',
                password: 'bitmascotIH99#',
                passphrase: 'bitmascotIH99#',
                privateKey: fs.readFileSync('private_key/PuttyPrivateKey.ppk')
            }).then(() => {
                return sftp.list('/var/log/tix/files/');
            }).then((data) => {
                console.log(data[0], 'the data info');
                let name = data[0].name;
                cmd.run('"C:\\Program Files (x86)\\WinSCP\\WinSCP.com" ^ /ini=nul ^ /command ^ "open sftp://imrul:bitmascotIH99%%23@13.93.0.186/ -hostkey=""ssh-ed25519 256 82:0f:8e:9e:95:3a:d4:de:8d:2f:55:4a:d1:5c:e5:99"" -passphrase=""bitmascotIH99#"" -privatekey=""F:\\tradeix_credientials\\PuttyPrivateKey.ppk""" ^ "get /var/log/tix/files/'+name+' D:\\wscp\\"');
            }).catch((err) => {
